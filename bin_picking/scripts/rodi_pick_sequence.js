...
var GRIP_IN_CH  = { ready: 0, grasped: 1, error: 2 };    // IN_1(원점·대기 완료)→DI0 · IN_2(파지 완료)→DI1 · IN_3(에러)→DI2  ✅ 9/16 펜던트 입출력 화면 실측으로 확정(열림 IN_0 · 물음 IN_1 · 빈손 IN_2)
...
// 🚨🚨 [9/9 실물] 완료 신호가 **쓸 수 없는 상태**였다 — IN_1(대기 완료)은 한 번도 안 올라왔고, IN_2 는 **부품을 물었는데도 High**(교안:22 의 "파지 실패 = 설정위치에서 정지"로 판정됨 —
//   협력사 GUI 가 외측파지범위를 0.00 으로 놓아 정상 파지가 창 밖으로 나갔다). ⇒ 신호로 판정하면 **성공을 실패로 읽었다.**
// 🎉 [9/16 실물 · GUI 재설정 뒤 해소] 파지2 = 4.33 / 외측파지범위 85.00 / 속도 60 / 토크 50 으로 바꾸고 펜던트 입출력 상태 화면에서 실측:
//   **열림 완료 = D_GEN_IN_0 · 부품 물음 완료 = D_GEN_IN_1 · 빈손(에러) = D_GEN_IN_2** ⇒ 교안 IN_1/IN_2/IN_3 이 DI0/DI1/DI2 로 한 칸 밀려 매핑(GRIP_IN_CH 그대로).
//   69·45·17.5mm 세 부품 전부 물림 ⇒ 신호 판정 경로가 살아났다 ⇒ GRIP_FEEDBACK 기본을 'signal' 로 되돌린다(DRILL_REQUIRE_GRASP_SIGNAL 가드도 실동작).
//   🚨 단 "신호가 뜬다"는 9/16 GUI 세팅 기준이다 — GUI 를 다시 만지면(외측파지범위·설정위치) 판정 창이 바뀐다 · 첫 실행에서 IN_1 이 안 뜨면 'none' 으로 내리고 눈으로.
//   🚨 [9/16] GUI 연결을 해제한 뒤 그리퍼가 DO·빨간 버튼에 전부 무반응이었다 → **그리퍼 전원 재투입으로 즉시 복구** ⇒ GUI 쓰고 나면 전원 껐다 켠다(절차).
var GRIP_FEEDBACK = 'signal';   // 'signal'(9/16 실물 · IN_0/IN_1/IN_2 로 판정 · 기본) | 'none'(9/9 방식 · 눈으로 · 신호가 안 뜰 때 내린다)
...
// 🚨 단위 = payload 는 kg(숫자) / center 는 mm(객체 {x,y,z}) — 배열이 아니다 (ko:129 시그니처)
// 🎉 [9/8] "한계 1.5kg 초과" 미결은 근거 오류로 해소 — 1.5 는 한계가 아니라 설정값이고
//    ko:129 의 1.5 는 예제 주석이다. 실제 한계는 로봇 정격 10kg(무게중심 거리로 감쇠).
//    ⇒ 카메라 장착 후 총 2.2~3.5kg 이라 여유는 충분하나 "설정값 갱신"은 필수다.
//    상세 = /data/jtm/handover_0908/payload_계산_0908.md
// 🚨 안전측으로 잡는다 — 실제보다 **작게** 잡으면 충돌 감지가 오작동하고,
//    **크게** 잡으면 감지가 둔해질 뿐 멈추지 않는다 ⇒ 상한을 쓴다.
var PAYLOAD_TOOL = 1.3;         // 그리퍼만 [kg] — 견적서 1.2 + 여유 (140형 유추 1.25 · 85형 공식값 없음 [미확인])
var PAYLOAD_PART = 0.05;        // 부품 1개 [kg] — 태민님 9/8 "되게 가벼움" ⇒ 문서의 0.94 는 폐기(출처 불명이었다)
var PART_COM     = { x: 0, y: 0, z: 50 };   // 무게중심 (ko:129 예제 형식 · mm)
// 🎥 카메라를 팔에 달면 TOOL 을 2.6 으로 올린다 [9/8 계산 · payload_계산_0908.md]
//    = 그리퍼 1.3 + 브라켓 0.5(레진 출력물 · 태민님 상한 · 실제 약 0.13) + Blaze 0.68 + ace2 0.10
//    🟢 정격 10kg 대비 26% · CoG 도 4kg 곡선 안(3세대 매뉴얼 25쪽 Payload graph) ⇒ 여유 크다
//    🚨 카메라는 옆으로 붙어 무게중심이 x·y 로도 이동한다 ⇒ TOOL 쪽 CoM 도 갱신 대상
//       (현 펜던트 설정은 GRAVITY Z=150 으로 z 만 잡혀 있다)
//    🚨 외형도 커지므로 setToolBoundingBox 갱신 대상 (조립 후 174 x 80.6 x 73mm)
 ...
 *   2. ✅ GUI 설정값 = **기본값이 아니었다**(9/9 · 파지1 46.53 · 파지2 37.92 · 외측파지범위 0.00) → **9/16 파지2 를 4.33/85/0/60/50 으로 재설정** ⇒ 21종 전부 물릴 수 있게 됨(69·45·17.5 실물 ○)
 *   2'. ✅ 완료 신호 = 9/16 실측 열림 IN_0 · 물음 IN_1 · 빈손 IN_2 ⇒ GRIP_FEEDBACK 기본 'signal' 복귀 (9/9 "IN_1 안 뜸"의 원인 = 외측파지범위 0.00)
 *   2''. 🚨 GUI 연결 해제 뒤 그리퍼 먹통 → 전원 재투입으로 복구(9/16) ⇒ GUI 뒤엔 전원 껐다 켠다
