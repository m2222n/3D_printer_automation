"""
⚠️⚠️ 폐기 예정 (DEPRECATED, 2026-07-30) — 빈피킹에 쓰지 말 것
================================================================
이 파일의 레지스터 맵은 **7/27 한화 매뉴얼 확정판과 정면 충돌**한다. 4/15에 교육 1차
내용으로 추정해 만든 것이고, 이후 확보한 예승님 운영 코드·매뉴얼과 어긋난다.

  | 이 파일           | 확정판(7/27)                        |
  |-------------------|-------------------------------------|
  | 131~137 = 좌표    | 131~135 = **이송 파라미터**(설비ID·시간) |
  | 150 = 로봇→PC 상태 | 150 = **PC→로봇 송신 트리거**(방향 반대) |
  | 151 = seq echo    | 151 = **PC Ready**                  |
  | 200/206 없음      | 200 Robot Ready / 206 Robot Moved   |

⭐ **역할도 반대다**: 이 파일은 비전PC를 *서버*로 띄우고 로봇이 읽어가는 전제인데,
   확정판 8단계는 **PC가 클라이언트로 로봇에 쓰는** 구조다
   (`sequence_service/app/cell/modbus_protocol.py:58 ModbusTcpClient`).

🔴 지우지 않는 이유 = `bin_picking/src/main_pipeline.py:51`이 아직 import한다.
   main_pipeline 정리 시 함께 제거할 것.

✅ **빈피킹은 대신 이것을 쓴다**:
   - 인코딩: `bin_picking/src/communication/pick_encoder.py`
   - 8단계 핸드셰이크: 예승님 `modbus_protocol.py:execute_command` **재사용**
     (레지스터를 전부 인자로 받게 설계돼 있음 = 재사용 의도)
   - 시뮬 검증: `bin_picking/tests/fake_robot_modbus.py`
   - 상세: `memory/project_modbus_pick_layer_0730.md`

⚠️ 아래 `mm_to_int16`은 범위 초과를 max/min으로 **조용히 클램프**한다 = 잘린 좌표로
   로봇이 엉뚱한 곳으로 가는데 에러가 없다. `pick_encoder`는 예외를 던진다.
