#!/bin/bash
# 7/4 cam_near 재학습 — 실측 실거리 45~50cm 정합 (7/3 미팅 지시)
# 데이터: dataset_2denc_camnear_01 (cam_h 0.43~0.57m + 부품 per-scene 0-1 정규화)
#   → train/test 동일 전처리 (조교 "test 노멀라이즈=train도 똑같이").
# CAD memory bank/3D encoder는 6/23 산물 재사용 (STEP5 스킵).
# warmup(det 30ep) → joint(VQ 100ep) → 합성 test + 실측 100장 eval (⭐F1 포함).
set -u
...
LOG=$RUNS/retrain_camnear_0704.log
MAX_RETRY=2
NAME=camnear
DATA=$ROOT/data/dataset_2denc_camnear_01

echo "===== cam_near 재학습 시작 $(date) =====" | tee -a $LOG
...
run_stage () {
  ...
  while [ $try -le $MAX_RETRY ]; do
    ...
    echo "[$SNAME] ⚠️ 시도 $try 실패, 재시도" | tee -a $LOG
...
echo "===== cam_near 재학습 종료 $(date) =====" | tee -a $LOG
