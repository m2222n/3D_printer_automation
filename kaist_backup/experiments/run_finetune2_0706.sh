...
C=$MN/data/real_labelme_dataset_C200_fgclean
OUT=/workspace/cadence/runs/C200_real_finetune_0706
LOG=/workspace/cadence/runs/C200_real_finetune_0706.log
echo "===== 2차 fine-tune (200장 train120) 시작 $(date) =====" > $LOG
...
echo "===== 학습 종료, test 50장 평가 $(date) =====" >> $LOG
cd $MN
REAL_ROOT=$C LABEL_DIR=./data/real_labels \
CKPT=$OUT/best.pt \
OUT_DIR=/workspace/cadence/eval_C200_test_s050_nms030_iou025 \
SCORE_THRESH=0.50 NMS_THRESH=0.30 IOU_THRESH=0.25 \
  bash ./scripts/eval_C_real_test.sh >> $LOG 2>&1
echo "===== 평가 종료 $(date) =====" >> $LOG
echo ">>> 결과:" >> $LOG
grep -oE "\"(f1_micro|precision_micro|recall_micro|mean_matched_iou_macro)\": [0-9.]*" /workspace/cadence/eval_C200_test_s050_nms030_iou025/eval_real_metrics.json >> $LOG 2>&1
