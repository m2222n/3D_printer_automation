...
# --- 카메라 (7/2 cam_sweep: 실측 정합 위해 거리 다양화 + FOV 고정) ---
# 기존 2denc: cam_h 0.36~0.44m 고정 + FOV를 거리에 맞춰 조정 → 부품이 항상 화면을 꽉 채움
#            → 실측(원거리·작게 찍힘)과 형상 프로파일 불일치 (7/1 probe 발견).
# 변경: cam_h 0.4~1.0m로 스윕(태민 "팔 뻗은 정도"=CAD 역산 0.8~1m 정합)
#       + FOV는 기준거리(0.42m)로 고정 → 멀수록 부품이 화면에서 작아지고 곡면 프로파일 강해짐.
#   ⚠️ 정규화가 절대 depth 배율은 흡수하므로(7/2 (A) 증명) 목적 = "픽셀 스케일/원근 프로파일" 정합.
# 7/3 hi 버전: camsweep(0.4~1.0m)이 실측 5%→39.6%로 진범=스케일 확증. 다만 부품이
#   여전히 실측(407px)보다 큼(1530px). → 상한을 1.6m로 올려 더 작은 부품(원거리) 커버.
#   부품 완전소실 방지 위해 하한도 0.5m로 약간 올림(너무 가까운 근접은 이미 39.6% 기여했으므로 축소).
CAM_H_MIN, CAM_H_MAX = 0.50, 1.60
