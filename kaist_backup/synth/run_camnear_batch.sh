#!/usr/bin/env bash
# 7/4 cam_near 배치 러너 — 실측 실거리 45~50cm 정합 합성 1000장 (cam_h 0.43~0.57m, FOV 고정)
...
SCRIPT=/home/jtm/kaist_project/synth/gen_one_2denc_camnear.py
BLENDER=/data/jtm/blender
OUT=/data/jtm/synth_out/dataset_2denc_camnear
NPZ=$OUT/npz
LOG=$OUT/run.log
...
mkdir -p "$NPZ"
echo "=== cam_near(45~50cm) batch 시작: N=$N, workers=$WORKERS, out=$OUT ===" | tee -a "$LOG"
date | tee -a "$LOG"
