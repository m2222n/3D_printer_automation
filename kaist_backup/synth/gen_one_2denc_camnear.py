...
# --- 카메라 (7/4 cam_near: 실측 실거리 45~50cm 확정 정합 + FOV 고정) ---
# 기존 2denc: cam_h 0.36~0.44m 고정 + FOV를 거리에 맞춰 조정 → 부품이 항상 화면을 꽉 채움.
# 7/2 cam_sweep: cam_h 0.4~1.0m 스윕(CAD 역산 "0.8~1m" 추정) → 실측 5%→39.6%(8배).
# ⭐ 7/3 미팅: 실측 부품↔카메라 실거리 = **약 45~50cm 확정**(태민님 현장 측정).
#    기존 스윕 0.4~1.0m는 실측(0.45~0.5)보다 평균적으로 멀다 → 실거리 근처로 좁힘.
# 변경: cam_h 0.43~0.57m (실측 45~50cm ± 여유, 부품높이·팔 변동 흡수). FOV 기준거리 0.42m 고정.
#   ⚠️ robust_normalize(median-subtract + p95-p05)가 절대 depth 배율은 흡수(7/2 (A) 증명).
#      따라서 목적 = "원근 프로파일(부품 픽셀 크기/왜곡)"을 실측 실거리에 정합시키는 것.
#   ⭐ train(합성)과 test(실측)가 같은 촬영 높이 → 같은 정규화 프로파일 (조교 "동일 높이 정규화" 지시).
CAM_H_MIN, CAM_H_MAX = 0.43, 0.57
FOV_REF_H = 0.42                                  # FOV 고정 기준 높이(기존 중앙값 근처)
cam_h = float(rng.uniform(CAM_H_MIN, CAM_H_MAX))
RES = 512
bproc.camera.set_resolution(RES, RES)
fov = 2 * math.atan((BIN_HALF * 1.4) / FOV_REF_H)  # ⭐ cam_h와 무관하게 고정
...
scene_meta = {
    ...
    "cam_h": cam_h,                # 7/2 cam_sweep: 카메라 높이(m) 기록
    "fov_ref_h": FOV_REF_H,        # FOV 고정 기준 높이
    "variant": "cam_near_0704_45to55cm",
}
