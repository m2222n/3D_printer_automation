...
def matched_metrics(
    ...
    # 배치 전체에서 "예측(positive)" 수 = no-object 아니고 최대확률 ≥ score_thresh
    ...
    sums["pred_positive"] += float(is_positive.sum().item())
    ...
    for b, (src_cpu, tgt_cpu) in enumerate(indices):
        ...
        # --- TP: 매칭된 예측이 (score positive) AND (클래스정답) AND (mask IoU≥thr) ---
        matched_scores = scores[b, src]
        tp = (matched_scores >= score_thresh) & cls_ok & (mask_iou >= iou_thresh)
