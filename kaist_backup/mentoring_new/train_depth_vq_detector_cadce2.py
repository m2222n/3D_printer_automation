...
from depth_vq_detector.depth_preprocess import input_channels_for_mode, parse_float_range
...
def build_dataset(source: str, args: argparse.Namespace, cad_id_to_index: dict[str, int], train: bool) -> DepthInstanceDataset:
    train_range = parse_float_range(args.train_depth_median_range)
    val_range = parse_float_range(args.val_depth_median_range) if args.val_depth_median_range is not None else train_range
    depth_range = train_range if train else val_range
    avg_pool_kernel = int(args.train_avg_pool_kernel if train else (args.val_avg_pool_kernel if args.val_avg_pool_kernel is not None else args.train_avg_pool_kernel))
    return DepthInstanceDataset(
        ...
        depth_median_range=depth_range,
        depth_median_random=bool(train and args.randomize_train_depth_median),
        robust_depth_median_range=parse_float_range(args.train_robust_depth_median_range) if train and args.train_robust_depth_median_range is not None else None,
        robust_depth_prob=args.train_robust_prob if train else 0.0,
        avg_pool_kernel=avg_pool_kernel,
        avg_pool_valid_threshold=args.avg_pool_valid_threshold,
        pseudo_uint16_max_depth_m=args.pseudo_uint16_max_depth_m if train else None,
        noise_sigma_m=args.train_noise_sigma_m if train else 0.0,
        noise_rel_sigma=args.train_noise_rel_sigma if train else 0.0,
        random_dropout_prob=args.train_random_dropout_prob if train else 0.0,
        boundary_dropout_prob=args.train_boundary_dropout_prob if train else 0.0,
        boundary_radius=args.train_boundary_radius,
        hole_prob=args.train_hole_prob if train else 0.0,
        target_valid_ratio_range=parse_float_range(args.train_valid_ratio_range) if train and args.train_valid_ratio_range is not None else None,
...
def build_criterion(args: argparse.Namespace, cad_codebook: torch.Tensor | None) -> SetCriterion:
    ...
    weight_dict = {
        ...
        "loss_mask": 2.0,
        "loss_dice": 2.0,
        "loss_cad_ce": 2.0 if cad_loss_on else 0.0,
...
def main() -> None:
    ...
    parser.add_argument(
        "--train_depth_median_range",
        default=None,
        help="Input-only pseudo depth distance simulation. Example: 0.45,0.55 or 0.35,0.55. "
             "The valid-depth median is shifted into this metric range during training.",
    )
    parser.add_argument(
        "--val_depth_median_range",
        default=None,
        help="Validation depth median range. If omitted, uses --train_depth_median_range midpoint deterministically.",
    )
    parser.add_argument(
        "--randomize_train_depth_median",
        action="store_true",
        help="Sample a random target median inside --train_depth_median_range per training sample.",
    )
    parser.add_argument(
        "--train_robust_depth_median_range",
        default=None,
        help="Optional robustness range sampled with --train_robust_prob, e.g. 0.35,0.45.",
    )
    parser.add_argument(
        "--train_robust_prob",
        type=float,
        default=0.0,
        help="Probability of sampling --train_robust_depth_median_range instead of --train_depth_median_range.",
    )
    parser.add_argument(
        "--train_avg_pool_kernel",
        type=int,
        default=1,
        help="Odd kernel for height-preserving valid-aware edge blur during training. 1 disables it. Legacy flag name kept for compatibility.",
    )
    parser.add_argument(
        "--val_avg_pool_kernel",
        type=int,
        default=None,
        help="Odd kernel for validation input. If omitted, uses --train_avg_pool_kernel.",
    )
    parser.add_argument(
        "--avg_pool_valid_threshold",
        type=float,
        default=0.05,
        help="After valid-aware edge blur, pixels with blurred valid support <= this threshold are reset to invalid/zero. 0.03~0.10 gives a thin soft edge; 0.5 keeps a tighter silhouette.",
    )
    parser.add_argument("--pseudo_uint16_max_depth_m", type=float, default=None, help="Simulate real uint16 depth export on pseudo train inputs, e.g. 10.0 for raw/65535*10m.")
    parser.add_argument("--train_noise_sigma_m", type=float, default=0.0, help="Absolute Gaussian depth noise in meters for pseudo train inputs, e.g. 0.0015.")
    parser.add_argument("--train_noise_rel_sigma", type=float, default=0.0, help="Relative Gaussian depth noise proportional to depth, e.g. 0.002.")
    parser.add_argument("--train_random_dropout_prob", type=float, default=0.0, help="Randomly invalidate this fraction of valid pseudo depth pixels during training.")
    parser.add_argument("--train_boundary_dropout_prob", type=float, default=0.0, help="Drop valid pixels near synthetic object boundaries with this probability.")
    parser.add_argument("--train_boundary_radius", type=int, default=2, help="Radius in pixels for boundary dropout band.")
    parser.add_argument("--train_hole_prob", type=float, default=0.0, help="Approximate random holes per 10k valid pixels for pseudo train inputs.")
    parser.add_argument("--train_valid_ratio_range", default=None, help="Optionally downsample pseudo valid pixels to match real crop valid ratio, e.g. 0.04,0.08.")
    parser.add_argument("--random_seed", type=int, default=42, help="Seed for deterministic depth median randomization per scene.")
    ...
    train_dataset = build_dataset(train_source, args, cad_id_to_index, train=True)
    ...
    if val_source is not None:
        val_dataset = build_dataset(val_source, args, cad_id_to_index, train=False)
