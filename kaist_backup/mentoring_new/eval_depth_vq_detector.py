...
def main() -> None:
    ...
    parser.add_argument("--depth_median_range", default=None, help="Optional deterministic median shift, e.g. 0.45,0.55. If omitted, checkpoint val/train range is used when present.")
    parser.add_argument("--avg_pool_kernel", type=int, default=None, help="Optional input avg-pool kernel. If omitted, checkpoint val/train kernel is used when present.")
    parser.add_argument("--center_keep_frac", type=float, default=None, help="Keep central fraction in x/y. 0.6667 keeps [1/6,5/6].")
    parser.add_argument("--center_roi", default=None, help="Optional ROI y0,y1,x0,x1 fractions.")
    ...
    eval_range = args.depth_median_range
    if eval_range is None:
        eval_range = ckpt_args.get("val_depth_median_range") or ckpt_args.get("train_depth_median_range")
    eval_range = parse_float_range(eval_range)
    eval_pool = args.avg_pool_kernel
    if eval_pool is None:
        eval_pool = ckpt_args.get("val_avg_pool_kernel")
        if eval_pool is None:
            eval_pool = ckpt_args.get("train_avg_pool_kernel", 1)
    ...
    dataset = DepthInstanceDataset(
        ...
        depth_median_range=eval_range,
        depth_median_random=False,
        avg_pool_kernel=int(eval_pool or 1),
        avg_pool_valid_threshold=ckpt_args.get("avg_pool_valid_threshold", 0.05),
        center_keep_frac=args.center_keep_frac,
        center_roi=args.center_roi,
