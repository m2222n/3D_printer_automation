...
def ensure_gt_display_defaults(gt_objects: list[dict[str, Any]] | None) -> list[dict[str, Any]]:
    """Ensure GT objects have stable idx/color/cad_name fields for drawing.

    LabelMe objects are loaded outside this module and may only contain
    raw_label/class_id/cad_name/mask/bbox.  This helper makes them compatible
    with the scene-npz visualization path.
    """
    out: list[dict[str, Any]] = []
    for n, obj in enumerate(gt_objects or [], start=1):
        item = dict(obj)
        item.setdefault("idx", n)
        item.setdefault("color", deterministic_color(int(item["idx"]) - 1))
        if "cad_name" not in item:
            item["cad_name"] = canonical_cad_display_name(item.get("raw_label", item.get("stl", "cad?")))
        item.setdefault("class_id", -1)
        out.append(item)
    return out
...
def draw_gt_panel(depth: np.ndarray, scene_npz: str | Path | None, title: str, gt_objects: list[dict[str, Any]] | None = None) -> Image.Image:
    base = _base_rgb(depth)
    overlay = base.astype(np.float32).copy()
    if gt_objects is None and scene_npz is not None:
        gt_objects = build_gt_display_objects(depth, scene_npz)
    gt_objects = ensure_gt_display_defaults(gt_objects)
    for gt in gt_objects:
...
def save_prediction_visualization(
    *,
    ...
    gt_objects: list[dict[str, Any]] | None = None,
    gt_title: str = "Ground truth overlay",
    ...
    if include_gt and gt_objects is not None:
        panels.append(draw_gt_panel(depth, scene_npz, gt_title, gt_objects=gt_objects))
