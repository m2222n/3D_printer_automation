...
def open_blaze(ip: str, short_range=True):
    ...
    if short_range:
        # ⭐ ShortRange 강제 적용 + 실제값 재확인 (7/6: LongRange로 찍혀 g2·g3 품질저하 발견).
        # 설정 후 반드시 다시 읽어 검증. 실패하면 크게 경고(조용히 넘어가지 않음).
        ok = _try("OperatingMode=ShortRange", lambda: cam.OperatingMode.SetValue("ShortRange"))
        try:
            cur = cam.OperatingMode.GetValue()
            if cur == "ShortRange":
                print(f"  ✅ OperatingMode 확인 = {cur} (근접 45~50cm 정합)")
            else:
                print(f"  🔴🔴 경고: OperatingMode = {cur} (ShortRange 아님! 근접 품질 저하 위험)")
                print(f"       → 촬영 중단 권장. set 성공여부={ok}")
        except Exception as e:
            print(f"  ⚠️ OperatingMode 재확인 실패: {e}")
