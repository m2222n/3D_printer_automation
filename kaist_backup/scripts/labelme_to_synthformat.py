...
# ---- CONFIG (조교 지시 반영 7/1) ------------------------------------
TARGET_RES = 512        # 합성과 동일
DEPTH_DIV  = 1000.0     # mm → m (BG_VALUE=0일 때는 정규화 전 내부 단위용)
BG_VALUE   = 0          # ⭐ 조교 지시(7/1): 배경 = 0 (NaN 아님). mask 밖 전부 0.
PNG_SCALE  = 2.0        # label_png = npy x2 → polygon 좌표 /PNG_SCALE

# ⭐ 조교 지시(7/1): 배경 0으로 밀고 부품만 0-1 정규화.
#   배경(빈 벽 등)이 섞이면 min/max를 잡아먹어 부품이 0 근처로 눌리는 문제 방지.
NORMALIZE_01 = True     # 부품 픽셀만 [0,1]로 스케일링, 배경 0 유지
NORM_MODE    = "per_scene"  # "per_scene"(장면별 min-max) | "fixed"(데이터셋 고정 상한)
NORM_FIXED_MIN_MM = 500.0   # NORM_MODE="fixed"일 때만 사용
NORM_FIXED_MAX_MM = 3500.0  # 실측 부품 depth 대략 범위(655~4299mm 관측) → 미팅 후 확정
# --------------------------------------------------------------------
...
def convert_one(json_path, npy_dir, name2cid):
    ...
    # depth: 배경 전부 0으로 밀고(조교 지시 7/1), 부품 픽셀만 채움
    depth = np.full((h, w), float(BG_VALUE), np.float32)
    part = inst_id > 0
    if NORMALIZE_01:
        # 부품 픽셀만 [0,1] 스케일링, 배경 0 유지
        pv = raw[part].astype(np.float32)  # mm
        if pv.size:
            if NORM_MODE == "fixed":
                lo, hi = NORM_FIXED_MIN_MM, NORM_FIXED_MAX_MM
            else:  # per_scene
                lo, hi = float(pv.min()), float(pv.max())
            rng = max(hi - lo, 1e-6)
            depth[part] = np.clip((pv - lo) / rng, 0.0, 1.0)
    else:
        depth[part] = raw[part] / DEPTH_DIV  # 정규화 안 할 때 m 단위
    ...
    meta = {
        ...
        "depth_units": "normalized_01" if NORMALIZE_01 else "meters",
        "background": "0",
        "norm_mode": NORM_MODE if NORMALIZE_01 else "none",
        "instances": instances,
