...
│  │  (27 routes) │  │  (실시간)    │  │  (15초 폴링)           │  │
│  └──────────────┘  └──────────────┘  └────────────────────────┘  │
└─────────────────────────────────────────────────────────────────┘
          │                                       │
          ▼                                       ▼
┌──────────────────┐                   ┌──────────────────────────┐
│  Formlabs Cloud  │                   │  공장 PC (Windows)        │
│   (Web API)      │                   │  ┌────────────────────┐  │
│   - 상태 모니터링 │                   │  │  PreFormServer      │  │
│   - 프린트 이력   │                   │  │  (Local API)        │  │
│   - 알림 발송     │                   │  │  - Scene 관리       │  │
│                  │                   │  │  - 프린트 전송       │  │
│                  │                   │  └──────────┬───────────┘  │
...
```
사무실 (501 오피스)                              공장 (도보 5분)
┌───────────────────────┐    WireGuard VPN    ┌───────────────────────┐
│  서버 (FastAPI :8085) │ ◀═══════════════▶  │  공장 PC              │
│  6000 서버 (개발)     │    10.145.113.x     │  PreFormServer :44388 │
│  5090 VM (운영)       │                     │  file_receiver :8089  │
└───────────────────────┘                     │  Form 4 x4 (WiFi)    │
                                              └───────────────────────┘
```
