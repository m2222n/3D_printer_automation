...
- 📏📏 **8/4(화) — 치수검사 STL 전수 실측. 추측으로 세운 전제가 두 번 뒤집힘**(설계 판단만 기록):
  ...
  - ✅ **홀 규격 = M3·M5로 확인** — 실측 검출값(ø3.0/3.2/3.4 및 ø5.0/5.5)이 M3 관통홀 ø3.2~3.4 · M5 ø5.2~5.5와 일치한다. ⭐**즉 실측이 도면과 어긋난 것이 아니라 구두 기록만 부정확했다는 확증.** 🎯**파생 = 공차 기준이 바뀐다** — 임의로 정한 ±0.1mm가 아니라 **"나사가 들어가는가"**가 기준이고, 이는 **한쪽으로 치우친 공차**다(크면 유격이 생기나 조립은 되고, 작으면 확실한 불량). ⭐**표준 나사 규격이 근거가 되므로 임의 공차보다 판정이 명확하고**, 검증도 핀 게이지 2~3개로 끝난다(ø3.2가 들어가고 ø3.4가 안 들어가면 실치수가 증명된다).
