...
- ✅⚠️ **7/29(수) 재택 — 두 가설이 실측으로 뒤집힘 + CPU 추론 환경 확보**:
  ...
  - 🤖 **로봇 연동 구조**: **맥 불가**(GPU 없음·로봇 대역 접근 불가·상주 불가) → 맥은 카메라 취득 전용. **Thor는 아직 발주 전**(사업계획 변경 승인 대기)=로봇 연동 최종 하드웨어 미도착. 재택 가능=좌표 출력 + **Modbus 시뮬레이터 핸드셰이크 검증**(pymodbus 보유) / 로봇 필수=핸드-아이 캘리브.
