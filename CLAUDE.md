...
| **Phase 4** | 장비 모니터링 (카메라 미확정) | 🟡 HIGH | 🔬 **7/14 촬영 → 7/15 분석**: ⭐**오후 대전환=진짜 신호는 조명색 아닌 "디스플레이 글자"**(카운트다운·`Finished`·메뉴). OCR 3엔진 전량 비교(%)=**rapidocr 우승**(MaixCAM 215장 52% > paddle 43% > easy 39%). 카메라=**OpenMV(512×320)는 완료판독 불가→MaixCAM급 고해상 이상 필요**(OpenMV·MaixCAM 확정아님). ⭐전량으로 보니 **wash 동작중은 89%로 됨**(카운트다운)·**wash 완료 0%·준비 낮음이 진짜 문제**=재촬영 필요(전면 불가 아님). 팀에서 PaddleOCR+ROI로 독립 진행(결론 일치). 세척기 뚜껑 공정 미결(코드 확인). ⚠️공정은 완료를 타이머로 판단(완료 IO 없음)→Vision 용도=검증+카운팅. **튜닝 전 내일 논의**. 상세 `memory/project_vision_state_detection_0715.md`, HTML `synth_out/monitoring_status_and_next_0715.html` |
