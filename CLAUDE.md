...
`/binpick/*`를 아직 서빙하지 않는다) / ⏸️**치수검사는 미착수**(측정 대상이 8/5로 드릴 전으로
바뀌어 스키마를 못 정한다 = 대표님 확인 대기).
...
| ③ 치수검사 | ⏸️ **측정 대상 미확정**(8/5로 드릴 전=출력물 치수로 변경)이라 스키마를 못 정한다 |
...
🚨 **공정 순서가 바뀌었다** = *"셀A가 끝나고 셀B 시작 전에 치수검사 먼저, 그 이후 바텀비전부터 작업 시작"* → ⭐**치수검사가 "드릴 전"이 되어 재는 것이 드릴 홀이 아니라 출력물 자체 치수가 된다.** 8/5에 적어둔 "드릴 전(출력물 치수) 갈래"가 이 케이스다. ⚠️**"드릴 진동에서 격리" 논리가 약해져** 치수검사 자리 근거를 재검토해야 한다. ⭐**다만 이 순서가 더 합리적이다** — 불량 출력물에 드릴을 뚫는 낭비를 막는다. ⏭️**무엇을 측정하는가**(외형·홀·휨·수축)를 확인해야 한다 — 8/3~8/4 광학 검산은 **ø3mm 홀 전제**였다.
